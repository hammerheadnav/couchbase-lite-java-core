...
public final class SQLiteSession {
    ...
    private Set<SQLiteConnection.PreparedStatement> mQueryStatementSet;
     ...
     * @throws com.couchbase.lite.v1.internal.database.sqlite.exception.SQLiteException if an error occurs.
    ...
    public SQLiteConnection.PreparedStatement beginQuery(String sql, Object[] bindArgs,
        ...
        SQLiteConnection.PreparedStatement statement =
                mConnection.executePrepareStatementNoRelease(sql, bindArgs);
        if (statement != null)
            getQueryStatementSet().add(statement);
        return statement;
    }

    public void endQuery(SQLiteConnection.PreparedStatement statement) {
        if (!getQueryStatementSet().contains(statement)) {
            throw new IllegalStateException("Must begin query before ending: " + statement.mSql);
        }

        try {
            mConnection.executeReleasePrepareStatement(statement);
        } finally {
            getQueryStatementSet().remove(statement);
            releaseConnection();
        }
    }

    private Set<SQLiteConnection.PreparedStatement> getQueryStatementSet() {
        if (mQueryStatementSet == null)
            mQueryStatementSet = new HashSet<SQLiteConnection.PreparedStatement>(1);
        return mQueryStatementSet;
    }
